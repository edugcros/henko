...
const buildShippingDataFromUser = user => {
  ...
  return {
    ...
    address: pickFirst(user.address, user.profile?.address, user.shippingAddress?.address),

    city: pickFirst(user.city, user.profile?.city, user.shippingAddress?.city),
...
const CheckoutPage = () => {
    ...
    if (paymentSuccess && isApprovedPayment(normalizedReduxPayment) && !paymentStatus.completed) {
      ...
      trackMetaEvent(
        'Purchase',
        {
          value: toNumber(confirmationSnapshot.totals.total, 0),
          currency: 'ARS',
          content_ids: confirmationSnapshot.items.map(item => String(item.productId)),
  ...
  const validateShippingForm = useCallback(() => {
    ...
    const required = ['firstName', 'lastName', 'phone', 'email', 'address', 'city', 'zipCode']
        ...
        <Fade in timeout={500}>
          ...
          >
                ...
                <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 3, height: '100%' }}>
                  ...
                  {confirmation.customer.address && (
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                      {confirmation.customer.address}
                      {confirmation.customer.city ? `, ${confirmation.customer.city}` : ''}
                      {confirmation.customer.province ? ` (${confirmation.customer.province})` : ''}
            ...
            <Alert severity="success" sx={{ borderRadius: 2, mb: 3 }}>
              Guardamos tu orden. El comercio coordinará la entrega a la dirección indicada.
            </Alert>
              ...
              {activeStep === 1 && (
                <Box component="form" noValidate>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <TextField
                        ...
                        onChange={event => handleShippingFieldChange('zipCode', event.target.value)}
                    ...
                    <Grid item xs={12}>
                      <TextField
                        ...
                        onChange={event => handleShippingFieldChange('address', event.target.value)}
                    ...
                    <Grid item xs={12} sm={6}>
                      <TextField
                        ...
                        onChange={event =>
                          handleShippingFieldChange('province', event.target.value)
                        }
                  ...
                  <FormControlLabel
                    ...
                    label={
                      <Typography variant="body2" color="text.secondary">
                        Quiero recibir ofertas y novedades por WhatsApp. Podés darte de baja cuando
                        quieras.
                      </Typography>
                    }
                  />
