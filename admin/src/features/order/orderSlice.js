...
export const updateOrderShipmentThunk = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      ...
      if (!response?.success) {
        return thunkAPI.rejectWithValue(response?.message || 'Error actualizando datos de envío')
      }

      return response
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error?.response?.data?.message || error?.message || 'Error actualizando datos de envío',
      )
    }
  },
)
