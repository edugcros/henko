    ...
    return (
      ...
      >
        ...
        <Typography variant="body2">
          {error.message || 'Todavía no hay datos suficientes para mostrar.'}
        </Typography>
      </Alert>
    )
  }
    ...
    <Box>
      ...
      {!loading && data?.definitions?.abandonedCart && (
        <Alert severity="info" sx={{ mb: 3 }}>
          Carrito abandonado: se cuenta cuando un carrito del tenant tiene
          productos y no se actualiza durante{' '}
          {data.definitions.abandonedCart.thresholdMinutes} minutos. Fuente:{' '}
          {data.definitions.abandonedCart.source}.
          {data.definitions.abandonedCart.dateField}.
        </Alert>
      )}
                      ...
                      data={[
                        {
                          name: 'Sesiones',
                          value:
                            data.ecommerce.funnel.sessions?.eventCount || 0,
                        },
                        {
                          name: 'Impresiones',
                          value:
                            data.ecommerce.funnel.productImpression?.sessions ||
                            0,
                        },
                        {
                          name: 'Clicks',
                          value:
                            data.ecommerce.funnel.productClick?.sessions || 0,
                        },
                        {
                          name: 'Ver Producto',
                          value:
                            data.ecommerce.funnel.viewItem?.eventCount || 0,
                        },
                        {
                          name: 'Add to Cart',
                          value:
                            data.ecommerce.funnel.addToCart?.eventCount || 0,
                        },
                        {
                          name: 'Checkout',
                          value:
                            data.ecommerce.funnel.beginCheckout?.eventCount ||
                            0,
                        },
                        {
                          name: 'Pago',
                          value:
                            data.ecommerce.funnel.paymentAttempt?.eventCount ||
                            0,
                        },
                        {
                          name: 'Compra',
                          value:
                            data.ecommerce.funnel.purchase?.eventCount || 0,
                        },
                      ]}
                      layout="vertical"
                    >
                  ...
                  {(data?.userBehavior?.topPages || [])
                    ...
                    .map(page => (
                      ...
                      >
                        ...
                        <Chip
                          size="small"
                          label={`${formatNumber(page.views)} vistas`}
                        />
                      </Box>
                    ))}
                  ...
                  {(data?.userBehavior?.topSearches || [])
                    ...
                    .map(search => (
                      ...
                      >
                        ...
                        <Chip
                          size="small"
                          label={`${formatNumber(search.count)} veces`}
                        />
                      </Box>
                    ))}
...
const AnalyticsConfigView = ({ onConfigurationSuccess }) => {
  ...
  const handleSave = async () => {
    ...
    if (!/^G-[A-Z0-9]{10,}$/i.test(formData.measurementId)) {
      enqueueSnackbar('Measurement ID inválido. Formato: G-XXXXXXXXXX', {
        variant: 'warning',
      })
      return
    }
    ...
    if (formData.serviceAccountJson && !isJsonValid) {
      enqueueSnackbar('Corrige el JSON del Service Account', {
        variant: 'warning',
      })
      return
    }
  ...
  const handleTest = async () => {
    try {
      ...
      if (response.data.configured) {
        enqueueSnackbar('Conexión exitosa con Google Analytics', {
          variant: 'success',
        })
      } else {
        enqueueSnackbar('GA4 no está completamente configurado', {
          variant: 'warning',
        })
      }
    } catch (error) {
  ...
  ...
            ...
            helperText={
              jsonError ||
              'Descargado desde Google Cloud Console > IAM & Admin > Service Accounts > Keys'
            }
            sx={{
          ...
          <Alert severity="warning" sx={{ mt: 2 }}>
            <strong>Importante:</strong> Este JSON contiene claves privadas. No
            lo compartas.
          </Alert>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Stepper activeStep={activeStep} orientation="vertical">
            {[
              {
                label: 'Crear propiedad GA4',
                content:
                  'Andá a analytics.google.com → Admin → Create Property',
              },
              {
                label: 'Obtener Measurement ID',
                content: 'Admin → Data Streams → copiá el ID (G-XXXXXXXXXX)',
              },
              {
                label: 'Crear Service Account',
                content:
                  'console.cloud.google.com → IAM & Admin → Service Accounts → Create',
              },
              {
                label: 'Descargar JSON',
                content: 'Service Account → Keys → Create New Key → JSON',
              },
              {
                label: 'Agregar permisos en GA4',
                content:
                  'Analytics → Admin → Property Access Management → Add user (email del service account)',
              },
              {
                label: 'Configurar aquí',
                content:
                  'Pegá el Measurement ID y el JSON completo en este formulario',
              },
            ].map((step, index) => (
              <Step key={index}>
...
const Dashboard = () => {
  ...
  return (
    <Box>
      ...
      <AnalyticsDashboardView onNotConfigured={handleNotConfigured} />
    </Box>
  )
}
