...
const CouponsPage = () => {
  ...
  const handleRestore = useCallback(async (coupon) => {
    try {
  ...
  const handleHardDelete = useCallback(async (coupon) => {
    const id = coupon?.id || coupon?._id
    if (!window.confirm(`⚠ ELIMINACIÓN PERMANENTE: "${coupon.code}"\nEsta acción no se puede deshacer.`)) return
    try {
  ...
  const handleClone = useCallback(async (coupon) => {
    try {
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ minHeight: 400 }}>
        {isLoading && !coupons.length ? (
          <Stack alignItems="center" sx={{ py: 10 }}>
            <CircularProgress />
            <Typography sx={{ mt: 2 }} color="text.secondary">Cargando cupones...</Typography>
          </Stack>
        ) : (
          <CouponList 
            coupons={coupons}
      ...
      <Modal isOpen={showBulk} onClose={() => setShowBulk(false)} title="Generador de Cupones" size="medium">
        <BulkGenerator 
          onSuccess={(res) => {
            setShowBulk(false);
            dispatch(getAllCoupons(filters));
