...
const Enquiries = () => {
  ...
  const handleSendReply = () => {
    ...
    .catch((err) => {
      ...
      toast.error(`Error: ${err || 'No se pudo conectar con el servidor'}`, { id: loadingToast });
    });
  };
  ...
  return (
    <Box p={4} sx={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems="center" mb={4} spacing={2}>
        <Box>
          <Typography variant="h4" fontWeight={800} color="#1A2027" sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          <Chip label={`Total: ${totalEnquiries}`} sx={{ fontWeight: 'bold', bgcolor: '#fff', border: '1px solid #ddd' }} />
      ...
      {isLoading && (
        <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" my={10}>
          ...
          <Typography sx={{ mt: 2 }} color="text.secondary">Cargando consultas...</Typography>
        </Box>
      )}

      {!isLoading && enquiryList.length === 0 && (
        <Paper sx={{ p: 10, textAlign: 'center', borderRadius: 4, border: '2px dashed #ddd', bgcolor: 'transparent' }}>
          <Typography variant="h6" color="text.secondary">No hay consultas nuevas.</Typography>
        </Paper>
      )}
          ...
          <Grid item xs={12} sm={6} md={4} key={enquiry._id}>
            <Fade in={true}>
              ...
              >
                <Box sx={{ 
                  height: 5, 
                  bgcolor: enquiry.status === 'Resolved' ? '#4caf50' : enquiry.status === 'In Progress' ? '#ff9800' : '#cfd8dc' 
                }} />

                <Box p={3}>
                  <Stack direction="row" justifyContent="space-between" mb={2}>
                    <Box>
                      <Typography variant="subtitle1" fontWeight={700}>{enquiry.name}</Typography>
                  ...
                  <Box sx={{ bgcolor: '#f5f7f9', p: 2, borderRadius: 2, mb: 3, minHeight: 60 }}>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
                    <Select
                      ...
                      size="small"
                      sx={{ borderRadius: 2, height: 35, fontSize: '0.8rem', minWidth: 120, bgcolor: '#fff' }}
                    >
      ...
      >
        ...
        <DialogActions sx={{ p: 2.5 }}>
          <Button onClick={() => setReplyOpen(false)} color="inherit">Cancelar</Button>
          <Button 
            variant="contained" 
            endIcon={<SendIcon />} 
      ...
      >
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button color="error" variant="contained" onClick={handleConfirmDelete} sx={{ borderRadius: 2 }}>
            Eliminar
          </Button>
        </DialogActions>
      </Dialog>
