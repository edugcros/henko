...
const pageStyles = {
  ...
  hero: {
    borderRadius: 24,
    padding: '24px 28px',
    background: 'linear-gradient(135deg, rgba(15, 23, 42, 0.98) 0%, rgba(30, 41, 59, 0.96) 48%, rgba(51, 65, 85, 0.92) 100%)',
...
const buildVariantPayload = (variant, index) => {
  const attributes = ensureVariantAttributes(variant)
  const cleanSku = normalizeSkuForPayload(variant.sku)
  const variantKey = buildVariantKey(attributes) || normalizeString(variant.key) || `variant-${index + 1}`
  const nombre = normalizeString(variant.nombre) || buildVariantName(attributes) || `Variante ${index + 1}`
...
const createSpecificationRow = (partial = {}, index = 0) => {
  ...
  return {
    ...
    key,
    label: normalizeString(partial.label || partial.name || humanizeKey(key)) || `Atributo ${index + 1}`,
...
const normalizeSpecSourceRows = product => {
  ...
  candidateObjects.forEach(source => {
    Object.entries(source instanceof Map ? Object.fromEntries(source) : source).forEach(([rawKey, rawValue]) => {
      const key = slugifyKeyPart(rawKey)
      if (!key || seen.has(key)) return
      seen.add(key)
      rows.push(createSpecificationRow({ key, label: humanizeKey(key), value: rawValue }, rows.length))
    })
  })
...
const normalizeSeoFromProduct = product => ({
  ...
  shortDescription: product?.seo?.shortDescription || product?.shortDescription || product?.summary || '',
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        {definitions.length ? (
              ...
              <Row gutter={[12, 12]}>
                <Col xs={24} md={8}>
                  ...
                  <Input
                    ...
                    onChange={event => updateAttribute(definition.id, { label: event.target.value })}
                    disabled={disabled}
                  />
                  {definition.name && (
                ...
                <Col xs={24} md={16}>
                  ...
                  <TextArea
                    ...
                    onChange={event => updateAttribute(definition.id, { valuesText: event.target.value })}
                    disabled={disabled}
                  />
          ...
          <Empty description="Todavía no definiste atributos de variantes">
            <Button type="primary" icon={<PlusOutlined />} onClick={addAttribute} disabled={disabled}>
              Crear atributo
            </Button>
          </Empty>
        )}
      ...
      <Space wrap>
        ...
        <Button type="primary" icon={<ThunderboltOutlined />} onClick={onGenerate} disabled={disabled}>
...
const EditProduct = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const productVariants = Array.isArray(normalizedProduct.variants) ? normalizedProduct.variants : []
  ...
  const handleToggleVariants = useCallback(
    checked => {
      ...
      if (checked && !variants.length) {
        ...
        const firstVariant = createVariantFromAttributes({ attributes, index: 0, basePrice: price })
  ...
  const handleAddVariant = useCallback(() => {
    ...
    const definitionName =
      slugifyKeyPart(variantDefinitions[0]?.name || variantDefinitions[0]?.label) || DEFAULT_VARIANT_ATTRIBUTE
  ...
  const handleGenerateVariants = useCallback(() => {
    ...
    const existingKeys = new Set(variants.map(variant => buildVariantKey(ensureVariantAttributes(variant))).filter(Boolean))
    const basePrice = normalizeNumber(form.getFieldValue('price'))
    const missingCombinations = combinations.filter(attributes => !existingKeys.has(buildVariantKey(attributes)))
  ...
  const syncVariantsWithServerProduct = useCallback((serverProduct, localVariants) => {
    ...
    return localVariants.map(localVariant => {
      ...
      const matched = serverVariants.find(serverVariant => {
        const serverAttributes = normalizeAttributes(serverVariant.attributes || serverVariant.combinacion || {})
  ...
  const handleFinish = async values => {
    ...
    try {
      ...
      if (hasVariants && variants.length > 0) {
        ...
        const syncedVariants = syncVariantsWithServerProduct(latestProduct || updatedProduct, variants)
        ...
        for (const variant of syncedVariants) {
          ...
          const matchingImage = mergedImages.find(
            img => img.url === variant.assignedImage.url || img.public_id === variant.assignedImage.public_id,
          )
      ...
      setVariantDefinitions(mergeVariantDefinitions(finalProduct?.variantAttributes || [], mappedFinalVariants))
          ...
          <Row gutter={[20, 20]} align="middle" justify="space-between">
            <Col xs={24} lg={15}>
              <Space direction="vertical" size={4}>
                <Badge count={normalizedProduct?.status || 'active'} style={{ backgroundColor: '#22c55e' }} />
                <Title level={2} style={pageStyles.heroTitle}>
                  {normalizedProduct.title ? `Editar ${normalizedProduct.title}` : 'Editor de producto'}
                </Title>
                <Paragraph style={pageStyles.heroSubtitle}>
                  Editor productivo para información comercial, SEO, ficha técnica, logística, galería y variantes sincronizadas con el storefront.
                </Paragraph>
              </Space>
            </Col>
              ...
              <Row gutter={[12, 12]} justify="end">
                ...
                <Col>
                  <Card size="small" style={{ minWidth: 128, borderRadius: 14 }}>
                    ...
                    <Title level={4} style={{ margin: 0 }}>
                      {hasVariants ? totalVariantStock : normalizeNumber(form.getFieldValue('stock'))}
                    </Title>
                  </Card>
                </Col>
              </Row>
            ...
            <Col xs={24} xl={16}>
              <Space direction="vertical" size={24} style={{ width: '100%' }}>
                  ...
                  <Row gutter={16}>
                    ...
                    <Col xs={24}>
                      <Form.Item name="shortDescription" label="Descripción corta">
                        <TextArea rows={3} maxLength={500} showCount placeholder="Resumen comercial para mostrar arriba de la ficha técnica." />
                      </Form.Item>
                    </Col>
                    <Col xs={24}>
                      <Form.Item name="metaDescription" label="Meta description">
                        <TextArea rows={3} maxLength={320} showCount placeholder="Descripción para SEO y previews." />
                      </Form.Item>
                    </Col>
                    <Col xs={24}>
                ...
                <Card
                  ...
                  extra={
                    <Button type="primary" icon={<PlusOutlined />} onClick={handleAddSpecificationRow}>
                      Agregar campo
                    </Button>
                  }
                >
                  ...
                  {!hasVariants ? (
                    ...
                    <>
                      ...
                      <Space style={{ width: '100%', justifyContent: 'space-between', marginBottom: 16 }} wrap>
                        ...
                        <Space wrap>
                          <Button icon={<ReloadOutlined />} onClick={handleSyncDefinitionsFromVariants}>
              ...
              <Space direction="vertical" size={24} style={{ width: '100%' }}>
                ...
                >
                  <Paragraph style={pageStyles.muted}>
                    Máximo {MAX_PRODUCT_IMAGES} imágenes. Las variantes pueden usar una imagen específica desde esta galería.
                  </Paragraph>
