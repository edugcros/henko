...
const PURPOSE_OPTIONS = [
  {
    value: '',
    label: 'Genérico',
    hint: 'Igual que hoy — proporción de la foto original',
  },
...
const ImageAiEditor = () => {
  ...
  const handleFileSelect = useCallback(
    event => {
      ...
      if (selected.size > MAX_FILE_SIZE) {
        enqueueSnackbar('La imagen no puede superar los 10 MB', {
          variant: 'warning',
        })
        return
      }
  ...
  const handleProcess = async () => {
    ...
    if (mode === MODES.VARIATION && !prompt.trim()) {
      enqueueSnackbar('Escribí un prompt describiendo los cambios', {
        variant: 'warning',
      })
      return
    }
    try {
      ...
      enqueueSnackbar(
        error.response?.data?.message || error.message || 'Error procesando la imagen',
        { variant: 'error' },
      )
    } finally {
      setProcessing(false)
  ...
  const handleAttachToProduct = async () => {
    ...
    try {
      ...
      enqueueSnackbar(`Imagen agregada a "${selectedProduct.title}"`, {
        variant: 'success',
      })
      setAttachTarget(null)
    } catch (error) {
      enqueueSnackbar(error.message || 'No se pudo agregar la imagen al producto', {
        variant: 'error',
      })
    } finally {
      setAttaching(false)
          ...
          <Stack spacing={2.5}>
            ...
            <Card sx={cardSx}>
              <CardContent sx={{ p: 2.5 }}>
                ...
                <Stack spacing={1}>
                  {[
                    {
                      value: MODES.REMOVE_BG,
                      icon: RemoveBgIcon,
                      title: 'Quitar fondo',
                      desc: 'Producto sobre fondo blanco limpio',
                    },
                    {
                      value: MODES.VARIATION,
                      icon: VariationIcon,
                      title: 'Generar variación',
                      desc: 'Cambiá fondo, escena o iluminación',
                    },
                  ].map(o => {
                    ...
                    return (
                      <Paper
                        ...
                        sx={{
                          ...
                          '&:hover': {
                            borderColor: sel ? V : alpha(V, 0.35),
                            bgcolor: alpha(V, sel ? 0.06 : 0.02),
                          },
                        }}
                      >
                        <Box
                          sx={{
                            ...
                            justifyContent: 'center',
                            flexShrink: 0,
                            background: sel ? GRAD_BTN : 'transparent',
                            bgcolor: sel ? undefined : alpha(V, 0.07),
                          }}
                        >
                    ...
                    {PURPOSE_OPTIONS.map(o => (
                      <Chip
                        ...
                        sx={{
                          ...
                          '&:hover': {
                            bgcolor: purpose === o.value ? V : alpha(V, 0.06),
                          },
                        }}
                      />
                    ))}
            ...
            <Button
              ...
              sx={{
                ...
                '&:hover': {
                  background: GRAD,
                  boxShadow: `0 8px 28px ${alpha(V, 0.35)}`,
                  transform: 'translateY(-1px)',
                },
                '&:active': { transform: 'translateY(0)' },
                '&.Mui-disabled': {
                  background: alpha('#94A3B8', 0.1),
                  color: alpha('#94A3B8', 0.45),
                  boxShadow: 'none',
                },
              }}
            >
                      ...
                      >
                        <Stack direction="row" alignItems="center" justifyContent="space-between">
                          ...
                          >
                            ...
                            >
                              {r.mode === MODES.REMOVE_BG ? (
                                <RemoveBgIcon sx={{ fontSize: 15, color: '#fff' }} />
                              ) : (
                                <VariationIcon sx={{ fontSize: 15, color: '#fff' }} />
                              )}
                            </Box>
                            <Box sx={{ minWidth: 0 }}>
                              <Stack direction="row" alignItems="center" spacing={0.75}>
                                ...
                                {r.purpose && (
                                  <Chip
                                    label={
                                      PURPOSE_OPTIONS.find(o => o.value === r.purpose)?.label ||
                                      r.purpose
                                    }
                                    size="small"
                                    sx={{
                          ...
                          <Stack direction="row" spacing={0.25}>
                            {[
                              {
                                tip: 'Usar en un producto',
                                icon: AttachIcon,
                                fn: () => handleOpenAttachDialog(r),
                                hc: V,
                              },
                              {
                                tip: 'Descargar',
                              ...
                              },
                              {
                                tip: 'Eliminar',
                                icon: DeleteIcon,
                                fn: () => setResults(p => p.filter(x => x.id !== r.id)),
                                hc: '#EF4444',
                              },
                            ].map(a => (
