...
const getProductDescription = product => {
  return clean(product.description || product.descripcion || product.shortDescription).slice(
    0,
    900,
  )
}
...
export const searchProductsTool = async ({
  ...
  const mongoQuery = searchCondition
    ? {
      ...baseQuery,
...
export const getProductDetailsTool = async ({ tenantId, productId, slug } = {}) => {
...
export const getPromotionsTool = async ({ tenantId, limit = 8 } = {}) => {
  ...
  try {
    ...
    return coupons.map(coupon => {
      ...
      const appliesToSpecificProducts = applicableProducts.length > 0
    ...
    console.warn('[AI_AGENT_TOOLS] promotions unavailable:', {
      tenantId,
